...
const MicrocauseCard = ({
  ...
  const { getMicroCauseDetails, getStrategyDetails, getEffectDetails, getGenericDetails } =
    useOntology();
        ...
        <ul className="flex flex-col gap-2">
          {items.map((id, idx) => {
            ...
            return (
              ...
              >
                <span className="font-bold block text-slate-800">{detail.name}</span>
                {detail.description && (
    ...
    >
      ...
      >
        <div className="flex-1 pr-2">
          <span className="font-bold text-lg text-indigo-900">{microDesc.name}</span>
      ...
      {isOpen && hasDetails && (
        <div className="mt-4 pt-4 border-t border-indigo-100/50 animate-in fade-in slide-in-from-top-2 duration-200">
          {renderList("Community Smells", mc.community_smells, getGenericDetails, Users, "text-purple-600")}
          {renderList("Riesgos", mc.risks, getGenericDetails, Shield, "text-red-500")}
          {renderList("Efectos", mc.effects, getEffectDetails, AlertCircle, "text-orange-500")}
          {renderList("Estrategias Preventivas", mc.preventive_strategies, getStrategyDetails, Target, "text-emerald-600")}
          {renderList("Estrategias Correctivas", mc.corrective_strategies, getStrategyDetails, Wrench, "text-blue-600")}
          {renderList("Indicadores", mc.indicators, getGenericDetails, Activity, "text-indigo-500")}
          {renderList("Métricas", mc.metrics, getGenericDetails, TrendingUp, "text-slate-600")}
        </div>
      )}
    </div>
  );
...
export const TextResultCard = ({
  ...
  const [isCardOpen, setIsCardOpen] = useState(!isDashboardMode);
    ...
    return (
      <div
        className={`p-5 flex flex-col gap-4 rounded-bl-none border-l-4 border-slate-300 ${
          isDashboardMode ? "bg-white border border-slate-100 shadow-sm" : "glass-panel"
        }`}
      >
        {result.cleaned_text && (
          <div
        ...
        <p className="text-sm text-slate-500">
          Este comentario ha sido clasificado como ruido operativo (Ej. logs de bots o
          mensajes automáticos) y no representa deuda social genuina.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`p-5 flex flex-col gap-4 rounded-bl-none border-l-4 border-indigo-500 transition-all ${
        isDashboardMode ? "bg-white border border-slate-200 shadow-sm" : "glass-panel"
      }`}
    >
      <div
        className={isDashboardMode ? "cursor-pointer group" : ""}
        onClick={() => isDashboardMode && setIsCardOpen(!isCardOpen)}
      >
        {result.cleaned_text && (
          <div
            className={`p-4 rounded-xl shadow-sm relative border ${
              isDashboardMode
                ? "bg-slate-50 border-slate-200 group-hover:bg-slate-100 transition-colors"
      ...
      {isCardOpen && result.microcauses && result.microcauses.length > 0 && (
        <div className="mt-2 space-y-3 pt-4 border-t border-slate-100 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {result.microcauses.map((mc, idx) => (
            <MicrocauseCard key={idx} mc={mc} isDashboardMode={isDashboardMode} />
          ))}
        </div>
      )}
    </div>
